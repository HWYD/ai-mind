...
graph LR
    ...
    B["共享层"]
    B1["共享 Step Operation"]
    B2["Runtime Guard"]
    B3["Final Artifact"]

    C["v0.2.0 StateGraph"]
    C1["node 节点"]
    C2["conditional edge 条件边"]
    C3["state patch 状态补丁"]
    C4["Graph 事件"]
...
```mermaid
graph TD
    Start["START"] --> Read["读取版本方案 readVersionPlan"]
    Read --> Ready["评估方案就绪度 evaluatePlanReadiness"]
    Ready --> Decision["规划决策 planningDecision"]

    Decision --> RouteDecision{"规划决策后的条件路由"}

    RouteDecision -->|ask_clarification| Ask["追问澄清 askClarification"]
    Ask --> EndAsk["END"]

    RouteDecision -->|stop_with_boundary_message| Stop["边界停止 stopWithBoundaryMessage"]
    Stop --> EndStop["END"]

    RouteDecision -->|read_optional_context| Optional["读取可选上下文 readOptionalContext"]
    Optional --> Strategy["决定 tasklist 拆分策略 decideTasklistStrategy"]

    RouteDecision -->|proceed or manual_review| Strategy

    Strategy --> Draft["生成 tasklist 草稿 v1 draftTasklistV1"]
    Draft --> Validate1["校验 tasklist v1 validateTasklistV1"]
    Validate1 --> Warning["判断 warning 处理方式 decideWarningDisposition"]
    Warning --> RouteWarning{"warning 处理后的条件路由"}

    RouteWarning -->|fixNow 大于 0| Revise["修正 tasklist 草稿 v2 reviseTasklistV2"]
    Revise --> Validate2["校验 tasklist v2 validateTasklistV2"]
    Validate2 --> Revision["评估修正效果 evaluateRevisionEffect"]

    RouteWarning -->|no auto revision| Revision

    Revision --> Artifact["输出最终 Artifact emitFinalArtifact"]
...
![AI Mind v0.2.0 Tasklist Agent 输出交互](../../assets/screenshots/version-images/v0.2.0/p3.png)
...
```mermaid
graph LR
    Exec["后端 StateGraph 执行"]
    N["node start / end"]
    R["route"]
    P["state patch summary"]

    Events["AI Mind Graph 事件"]
    ...
    Panel["AgentTracePanel Graph 时间线"]
