...
- v0.5.0 不配置代码签名；内部预览制品必须标记“内部预览、未签名、不得公开分发”，并在受控内部渠道与 SHA-256 校验信息一起提供

内部预览 desktop build 的 production Origin（`https://ai.hwyblog.cloud`）和产品 identity 是受版本控制的 build constants。开发时可传入明确的 localhost Origin；此能力必须由 `app.isPackaged === false` 强制隔离，不能出现在内部预览安装包的 UI、配置文件或环境回退中。v0.5.0 不配置或打开升级 URL；升级只由受控内部渠道人工交付。
...
2. 实现 `DesktopBuildConfig` 的 preview/dev 校验；写入 fuses、内部预览标识与 hash manifest 的生成/验证脚本。
3. 在 webapp 先实现 strict `GET /api/desktop/compatibility` contract 和 policy 测试；它不得读取 session、写 DB 或返回 upgrade URL。先按既有 server deployment 方式将此契约和 headers 部署、验证，再允许制作可分发桌面预览包。
...
6. 实现 work/recovery 两种窗口；work 无 preload，recovery 使用绑定 memory session 的 internal protocol、最小 preload 和 sender-validated IPC；恢复页只提供内部渠道升级说明，不提供外链。
...
10. 补 Windows/macOS CI/test lane、内部预览打包、预览标识/fuse/hash 验证及兼容/不可用/内部渠道升级说明/关闭恢复 smoke。
...
## 4. Server-first Internal Preview Procedure

`make:windows` is a non-distributable package check. Do not run `preview:make`, create a
manifest/hash, or hand an installer to the internal channel until the webapp changes are
deployed through one of the two existing server deployment routes and the same candidate
version passes the fixed-origin check:
...
Only then may the distributor create the same-commit unsigned Windows x64 and macOS
arm64 preview artifacts, platform-specific `desktop-release.json` files, and SHA-256
files, verify them against the real package fuses, and collect fresh/overlay-install
evidence. macOS first launch may require the documented Gatekeeper right-click/Open
step in Finder; do not disable Gatekeeper globally or remove quarantine attributes. This does not make the artifact signed or notarized. If the server rolls back past the
compatibility or document-header contract, stop distribution first. Existing desktop
clients must stay fail-closed; do not add an alternate Origin, HTTP fallback, or upgrade
URL.
...
| Internal preview artifact | Windows x64 与 macOS arm64、“内部预览、未签名、不得公开分发”标识、fuse 状态、hash manifest 与 artifact 一致；不生成 Intel/universal 制品，制品不进入公开正式发布渠道                                                                     |
...
| Server returns manual upgrade                       | 不加载 workspace；显示当前版本、最低版本和受控内部渠道升级说明，不提供应用内升级链接                                                                                                 |
| Manual overlay install newer internal preview build | 保留同一平台/产品 profile；重新启动后可用的会话持续可见                                                                                                                              |
...
- 不接入 Windows Authenticode、Apple Developer ID signing 或 notarization；macOS fuse 修改后仅允许 ad-hoc `codesign --sign -` 完整性步骤。用独立命令验证内部预览标识、fuses、artifact hash 和平台化 `desktop-release.json`。
- 服务器端 compatibility/header 改动仅通过既有生产部署路径发布；在固定 production Origin 验证候选桌面版本的 compatibility response、无 cookie 副作用、`/` 与 `/instant-mind` 的 CSP/security headers 后，才可以生成和分发对应 preview artifact；不要为 desktop 引入新的 server deploy route。
- 若需将 server 回退到不含 compatibility API 或 document security headers 的版本，先暂停对应 preview 的内部渠道分发；已安装客户端保持 fail closed，不增加 fallback。
- 确认内部预览安装包没有 secret、dev Origin、代理/TLS bypass、auto-update、telemetry endpoint、通用 native IPC，且带有未签名内部预览标识。
