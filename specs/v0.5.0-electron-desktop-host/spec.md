...
**Status**: 实现完成；公开 GitHub Pre-release 的运营验收待完成
...
- Q: v0.5.0 公开试用版的 AI Mind Desktop Host 应如何确定其受信线上服务地址？ → A: `Unsigned Experimental Preview` 安装包在构建时固定唯一的官方生产 HTTPS 服务地址，用户不能自行修改；开发调试才可显式配置本机地址，且该配置不得进入公开制品。
...
- Q: v0.5.0 的手动发布边界包含什么？ → A: 本版通过公开 GitHub Pre-release 发布明确标识为“Unsigned Experimental Preview”的可校验制品，并提供 SHA-256 与已脱敏的支持诊断信息。代码签名、自动更新、SSO 与自定义协议回调留给后续版本。
- Q: 桌面端发生故障时，已脱敏诊断信息应如何交给支持人员？ → A: 默认不上传；诊断仅在本机生成，由用户主动复制或导出后再交给支持人员。

### Session 2026-08-06

- Q: v0.5.0 新增 macOS 支持的范围是什么？ → A: 仅支持 Apple Silicon 的 macOS arm64，提供未签名的公开 Beta DMG；不支持 Intel x64 Mac，不制作 universal binary，不接入 Developer ID 签名或 notarization。
...
- v0.5.0 的发布平台范围扩展为 Windows x64 与 macOS arm64（Apple Silicon）。macOS 提供未签名的公开 Beta DMG；Intel x64 Mac、universal binary、签名和公证仍不在本版范围内。

## Current Final Scope and Execution State

v0.5.0 的最终交付范围是一个在线 Electron host：支持 Windows x64 Squirrel 安装器与 Apple Silicon macOS arm64 DMG，并通过公开 GitHub Pre-release 作为 `Unsigned Experimental Preview` 提供试用。Intel Mac、universal binary、Developer ID signing、notarization、自动更新和本地 AI Runtime 均不在范围内。

执行顺序以最终收口为准：完成已有基础能力、CI、规格资产和 macOS arm64 extension 后，维护者将同一 source commit 合并并部署到 `main`，运行 production verification；随后手动触发公开发布 Workflow，最后完成双平台 fresh-install smoke 与 sign-off。任务 ID 只用于追溯，不表示当前执行优先级。
...
3. **Given** 用户使用的是已配置的公开 Beta 版本，**When** 应用请求工作界面或功能数据，**Then** 请求只会发送到该版本声明的受信 AI Mind 服务。
...
**Why this priority**: 桌面应用比普通网页更接近用户设备；安全边界是公开试用及后续正式发布的前提。
...
- 已安装桌面版被受信服务判定为不兼容时，应用必须停留在本地兼容性失败状态，显示最低版本和 GitHub Pre-release 获取较新 `Unsigned Experimental Preview` 的说明；不得在应用内打开升级页面。
...
- **FR-002**: 系统必须在桌面应用的独立工作窗口中加载公开 Beta 版本预先声明的受信 AI Mind 在线服务，并拒绝任意未声明的工作界面来源；公开安装包必须在构建时固定唯一的官方生产 HTTPS 服务地址，用户不得自行修改该地址。开发调试可显式配置本机地址，但该配置不得进入公开安装包。该 Origin、产品 identity 或 release metadata 的变更必须由当前版本负责人批准，重新构建候选制品，并在同一 canonical spec 工作区同步相关 contract、plan、tasks 和 release evidence。
...
- **FR-011**: v0.5.0 的桌面安装包必须明确标记为“Unsigned Experimental Preview”，并通过公开 GitHub Pre-release 提供版本、平台 manifest 和 SHA-256 校验信息；本版本不得将该制品表述为已签名、已公证或可让操作系统识别发布者的正式发行物。代码签名与正式稳定版必须在后续独立版本完成。打包 fuse 必须与随 Electron 发布的启动资源兼容，并以实际可执行文件读取和启动 smoke 验证，不能只验证源配置。
- **FR-012**: 系统必须保持现有服务端密钥、数据库凭据、模型提供方凭据和内部服务凭据不进入桌面安装包、页面公开数据或用户可见日志。
- **FR-013**: 系统必须能判定 Desktop Release 与受信 AI Mind 服务是否兼容。若状态为不兼容，应用不得加载不受支持的工作界面，必须展示本地最低支持版本、GitHub Pre-release 手动升级说明和当前版本信息；本版本不得自行下载、安装更新或打开升级 URL。
...
- **FR-022**: v0.5.0 public-beta Desktop Release 必须具有稳定的产品身份与用户资料路径，并随制品提供可校验的版本与 SHA-256 完整性信息，以支持用户手动安装与问题回退定位；代码签名发布者信息不属于本版本承诺。
- **FR-023**: compatibility API 和 document security headers 必须先通过既有 server deployment 机制上线，并在固定生产 Origin 验证候选 Desktop Release 的 strict response、无 cookie 副作用和安全 headers；production verifier 必须精确接受 `style-src 'self' 'unsafe-inline'`，并拒绝 `style-src` 的 nonce/hash 与任意 `style-src-attr`。document CSP 必须继续严格限制脚本，而 Web document 与包内 Electron local document 的全部 CSS 必须通过上述指令生效。该 CSS 兼容例外不得扩展到 `script-src`、API 或静态资源响应，也不得新增远程样式来源、改变 Origin、权限、资源白名单或导航策略。验证通过后，维护者才能手动触发公开 GitHub Pre-release。候选版本负责人、server deploy operator 和 public-beta 发布负责人必须在不含用户资料的 evidence record 中共同关联同一 commit、installer、manifest、SHA-256 与公开 Release 说明。若 server 必须回退到不具备这些能力的版本，必须先暂停对应公开 Release，客户端保持 fail closed。
...
- **FR-025**: v0.5.0 必须为 Windows x64 生成 Squirrel 安装包，为 macOS arm64 生成 DMG 安装包；每个制品必须带有唯一的平台/架构标识、版本、SHA-256 和“Unsigned Experimental Preview”说明，并附至公开 GitHub Pre-release。macOS 首次启动可能被 Gatekeeper 阻止，配套验收说明必须记录用户主动的 Finder 打开步骤。
...
- **Desktop Release**: 一份面向特定操作系统和架构的 AI Mind 桌面公开 Beta 发行物，包含产品版本、受信服务配置和 SHA-256 校验信息；v0.5.0 不包含代码签名发布者保证。
- **Trusted AI Mind Origin**: 该桌面发行物被允许在主工作窗口中加载的官方线上服务地址范围。v0.5.0 公开安装包仅包含构建时确定的唯一官方生产 HTTPS 地址，用户不可编辑；开发调试的本机地址配置不属于公开发行物。
...
- **SC-011**: 公开 Beta 验收中，100% 的分发安装包和 GitHub Pre-release 配套说明均显著标明“Unsigned Experimental Preview”，并提供与实际安装包一致的 SHA-256 校验值。
...
- SC-011 的“分发安装包和配套发布说明”固定包含同一 commit 的 installer、`desktop-release.json`、SHA-256 文件、GitHub Pre-release URL 及安装后 native About 标识；任一项缺失、版本或 hash 不匹配即为候选失败。

## Assumptions

- v0.5.0 是需要联网的桌面入口；现有线上 AI Mind 服务继续负责聊天、受控任务、增量回答、持久化、外部能力与模型调用。
- 首版公开 Beta 支持 Windows x64 与 macOS arm64；macOS Intel x64、universal binary、Linux、离线运行和本地模型不属于本版本交付承诺。
...
- 用户仅通过 GitHub Pre-release 主动获得并安装新 Desktop Release；手动升级和回退的可定位性依赖稳定产品身份、版本信息与 SHA-256 校验信息，而不是应用内自动更新或代码签名。
...
- 开发调试可显式配置本机服务地址，但公开安装包不提供切换官方环境、填写自建服务地址或修改受信地址的入口。
...
- 不在 v0.5.0 支持用户填写任意服务地址、自建服务接入或公开安装包内的官方环境切换。
...
- 不在 v0.5.0 采购代码签名证书、接入托管签名服务，或将未签名试用制品表述为正式稳定发行物。
