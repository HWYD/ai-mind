...
- [x] T019 为 artifact manifest 的 source commit/版本字段、SHA-256、`public-beta`/未签名标识与实际 fuse 检查编写测试：`apps/desktop/tests/unit/release-artifact.test.ts`。
...
- [x] T031 [US5] 创建 `apps/desktop/src/recovery-renderer/index.html`、`apps/desktop/src/recovery-renderer/main.ts` 与 `apps/desktop/src/recovery-renderer/styles.css`，显示 safe error code、当前/最低版本、GitHub Pre-release 升级说明、重试、确认 reset 和诊断操作；不得引用外部资源或升级 URL。
...
**Independent Test**: 支持的公开 Beta 安装包在正常网络下 10 秒内进入独立 workspace，并完成一次普通聊天与流式回答，且不打开普通浏览器。
...
- [x] T040 [US1] 在 `apps/desktop/src/main/main.ts` 与 `apps/desktop/src/main/application-menu.ts` 实现 app lifecycle、single-instance lock、Squirrel startup 处理、窗口聚焦、仅在 compatibility 成功后 `loadURL(trustedOrigin)` 的启动链路，以及无论 workspace 或 recovery 状态均可用的原生 About/版本入口；该入口只由 main process 构造，显示 desktop version、`public-beta`、`unsigned` 与 fixed Origin，不加载远程内容、不打开升级 URL，也不向 workspace renderer 暴露 bridge。
...
**Purpose**: 接入 Windows CI、生产验证、公开 Beta 制品治理、文档与版本收口，确保没有绕开既有两条 server deployment route。
...
- [x] T060 更新 `scripts/validate/validate-test-lanes.mjs`、`scripts/validate/validate-ci-workflow.test.mjs` 与 `.github/workflows/ci.yml`，新增不持有生产 secret、不执行 server deploy 的 Windows x64 desktop job：锁定依赖安装、desktop unit、development Electron integration、不可分发的 `make:windows` 与 artifact/fuse/hash 验证；常规 CI 不得执行、命名、上传或标记可发布制品。公开 Beta Workflow 仅可在同一 commit 的 server compatibility API/document security headers 生产验证通过后手动执行。
...
- [x] T062 同步 `docs/architecture/production-deployment.md`、`specs/v0.5.0-electron-desktop-host/contracts/desktop-preview-release.md` 与 `specs/v0.5.0-electron-desktop-host/quickstart.md`，明确先 server 后公开 Beta Release、仅两条 server deploy route、暂停 GitHub Pre-release 后的 rollback 规则。
- [x] T063 创建 `docs/adr/0017-secure-electron-desktop-host.md` 与 `docs/architecture/desktop-host.md`，沉淀 online host、固定 Origin、零 bridge、profile/recovery 分离、compatibility gate、下载/外链、fuse 与未签名公开 Beta 边界。
- [x] T064 创建 `docs/versions/v0.5.0-electron-desktop-host.md`、`docs/releases/v0.5.0.md` 与 `docs/tasklists/v0.5.0-electron-desktop-host-tasklist.md`，说明已交付范围、Non-goals、公开 Beta 发布边界、手工升级、回退和验证证据。
- [x] T065 更新 `README.md` 与 `specs/v0.5.0-electron-desktop-host/quickstart.md`，使桌面能力、支持平台、公开 Beta 限制、开发/验证命令与真实 scripts 一致，不把未签名制品描述为正式签名发行物。
...
- [x] T069 完成 `speckit-analyze`、阶段工程审计与 `speckit-converge`，将发现与剩余任务回写 `specs/v0.5.0-electron-desktop-host/tasks.md`，再进行公开 Beta release closing。
...
- **Phase 9**：依赖所有计划交付的用户故事；生产 server 发布与公开 GitHub Pre-release 遵循 `contracts/desktop-preview-release.md` 的先后顺序。
...
4. 在此 checkpoint 做 Windows Safe MVP smoke；尚未完成高级能力、图像保存、会话滑动续期或公开 Beta 发布时，不得把它称为 v0.5.0 候选。
...
5. Phase 9 → Windows CI、生产 contract 验证和公开 GitHub Pre-release 制品收口。
...
## Phase 12: macOS arm64 Public Beta Extension

**Purpose**: 在不改变 online-host、零远程 bridge、固定 Origin 或 server-first 发布边界的前提下，为 Apple Silicon Mac 增加未签名 DMG 公开 Beta 制品；不支持 Intel Mac、universal binary、Developer ID signing 或 notarization。
...
- [x] T085 执行 `pnpm --filter @ai-mind/desktop lint`、`pnpm --filter @ai-mind/desktop typecheck`、`pnpm --filter @ai-mind/desktop test:stable`、CI validator tests、`git diff --check`，并在 `acceptance.md` 区分本地 Windows 验证、待运行的 macOS arm64 CI/DMG 验证与仍被 production verifier 阻塞的公开发布（FR-023、SC-012、SC-013）。

**Checkpoint**: macOS arm64 代码、契约、CI 与人工验收路线完整，但公开 GitHub Pre-release 必须等待 production verifier 通过。

---

## Phase 12.5: Pre-release Audit Remediation

**Purpose**: 在 source commit、server deploy、公开发布开始前，修复审计确认的 release verifier、local recovery 和实际 package-content audit 缺口；本阶段不执行最终发布门禁，也不生成或分发制品。
...
- [x] T110 修复 Electron 43 打包可执行文件在主进程启动前退出的问题：禁用与缺失 `browser_v8_context_snapshot.bin` 不兼容的 `LoadBrowserProcessSpecificV8Snapshot`，让源配置、fuse unit/artifact verifier、canonical specs、ADR 与 architecture docs 一致；已重建 Windows package、读取实际 fuse wire，并完成 startup smoke（FR-011、T068）。
...
- [x] T120 修复生产服务器实测发现的 port-probe 误判：当 Docker Compose 对仅 `expose` 的 PostgreSQL `5432` 或 project-assistant-service `8788` 输出 `invalid IP:0` 但以成功状态退出时，`verify-production.sh` 只接受符合 `host:port` 格式的输出，仍拒绝任何真实宿主机端口映射；补充 Linux deployment-host regression test 和生产部署文档，且不修改 Compose 网络边界或生产 env（FR-023）。

**Checkpoint**: 审计发现的本地实现与 release-verifier 缺口已关闭；公开发布仍必须等待维护者部署、production verifier 与双平台 fresh-install smoke。
...
- [ ] T073 仅在 T072 通过后，手动触发 `.github/workflows/desktop-public-preview.yml`，以同一 source commit、release tag 和 `production_verified=true` 构建并审计 Windows x64 与 macOS arm64 制品，创建公开 GitHub Pre-release（FR-001、FR-011、FR-012、SC-011；external owner）。
- [ ] T074 使用 T073 的公开制品完成 Windows x64 与 macOS arm64 fresh-install、checksum、SmartScreen/Gatekeeper、正常聊天和 recovery 手工矩阵；将脱敏环境、兼容性、启动时长与安全拒绝结果记入 `acceptance.md`（US1-US6、SC-001、SC-011、SC-012、SC-013；external owner）。
- [ ] T075 更新验收台账和角色 sign-off；只有所有 release gate 均为 Pass 时才将 v0.5.0 标记为公开 Beta 可体验（plan: release closing；external owner）。

---

## Phase 14: Public Beta Release Automation and Documentation

- [x] T115 [P] 将 `DesktopBuildConfig`、`DesktopPreviewManifest` 和相关单元测试的 distribution 固定为 `public-beta`，保留 `signing: unsigned`：`apps/desktop/src/main/build-config.ts`、`apps/desktop/scripts/release-artifact-utils.{mjs,d.mts}`、`apps/desktop/tests/unit/{build-config,release-artifact}.test.ts`（FR-011、FR-022）。
- [x] T116 [P] 在 `.github/workflows/desktop-public-preview.yml` 新增仅可 `workflow_dispatch` 的 Windows/macOS 原生构建、manifest/hash/package audit、artifact transfer 和 `gh release create --prerelease --target "$SOURCE_COMMIT"` 发布链路；以 `production_verified=true` 作为维护者确认门禁，不执行部署或读取生产 secret（FR-001、FR-002、FR-003、FR-004）。
- [x] T117 [P] 更新 `scripts/validate/validate-ci-workflow.test.mjs`，锁定公开发布 Workflow 的手动触发、确认门禁、Pre-release、source commit target 和部署隔离（FR-001、FR-003）。
- [x] T118 [P] 更新 `README.md`、`LICENSE`、`apps/desktop/tests/packaged/startup-smoke.md`、版本/发布/架构/ADR 文档和 canonical spec artifacts，保留既有需求与任务历史，仅将发布策略替换为公开 `Unsigned Experimental Preview`（FR-005、FR-008）。
- [x] T119 运行 desktop stable tests、全仓库 lint/typecheck、CI workflow validator 与 `git diff --check`，完成当前 canonical workspace 的 Spec Kit analyze/converge；外部发布门禁保留在 T071-T075（SC-002、SC-003）。
