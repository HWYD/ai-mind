...
| ------------------------------- | -------------------------------------------------------- | ----------------------------------------------------------------- | -------------------------------------------------------------------------------- | ----------------------------------------------------- |
| `DesktopBuildConfig`            | 内部预览 desktop build 的包内常量                        | internal preview 生命周期                                         | desktop version、固定 Origin、产品 identity、preview distribution 标记           | 用户配置、secret、升级 URL、任意 server URL、签名凭据 |
| `DesktopPreviewManifest`        | 打包后基于实际安装器生成的只读文件                       | 单个内部预览制品生命周期                                          | version、platform、Electron version、可信 Origin、preview/unsigned 标记、SHA-256 | secret、签名凭据、用户资料、任意下载地址              |
...
- v0.5.0 不保存、显示或打开任何升级 URL。`manual_upgrade_required` 只显示最低支持版本与“从受控内部渠道获取较新内部预览制品”的固定说明；兼容性 API 不提供升级 URL。
- `distribution` 和 `signing` 是 v0.5.0 内部预览的固定值；任何将其改为公开分发或已签名的构建必须在后续版本重新进行发布设计与验收。
...
- manifest 只能与显著标记“内部预览、未签名、不得公开分发”的安装器在同一受控内部渠道提供；它不替代公开发行所需的代码签名。
...
- `manual_upgrade_required` 不是 `unavailable` 的同义词：它必须显示最低支持 release 与内部渠道升级说明，但不提供应用内升级链接。
