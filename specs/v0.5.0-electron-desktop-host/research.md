...
## Decision 8: 公开 Beta 制品、hash 校验与手动升级，不做代码签名或 auto-update

**Decision**

v0.5.0 生成 Windows x64 与 macOS arm64 的未签名公开 Beta 制品，不采购证书，也不接入 Developer ID、notarization 或公开代码签名服务。所有安装包和说明必须明确标记 `Unsigned Experimental Preview`、`public-beta` 与 `unsigned`，并通过 GitHub Pre-release 提供。每个制品都生成并验证平台化 hash manifest；同一产品 identity 的手动覆盖安装保留 profile；不引入 `autoUpdater` 或任何后台更新检查。macOS 在 flip fuses 后仅执行 `codesign --force --deep --sign -` 的 ad-hoc 重签名以恢复本地可执行完整性；它不携带发布者身份，制品仍是 unsigned public Beta，并必须提供 Gatekeeper 的人工打开说明。
**Rationale**

- 未签名制品可作为公开 Beta 提供，但不能被描述为正式签名发行；代码签名基础设施推迟到具备合格主体、预算和服务选择的后续版本。
- manifest 为下载者提供来源定位和手动 hash 校验的最小可读证据，但不替代正式发行所需的代码签名。
- 自动更新还需要更新源、签名、分阶段策略、回滚与事故响应；本版先保证手动覆盖安装不丢失 profile。
  **Alternatives Rejected**

- 不经生产验证便公开发布：不满足 server-first 约束。
- 在 v0.5.0 采购证书或接入托管签名：超出公开 Beta 的最小复杂度。
- 先实现自动更新：明显超出 scope，且会在安全关键路径引入额外服务和隐式下载。
- 使用可提交到仓库的 PFX/password：不可接受；所有签名 secret 必须留在受保护环境。
  **References**
...
## Decision 10: Windows/macOS CI 分层验证，生产 fuse 包不做 Playwright attach

**Decision**

保留现有 Ubuntu web CI，并新增 Windows x64 与 macOS arm64 desktop 车道。desktop pure-policy/unit 在常规 test runner 执行；主进程交互使用未打包 development Electron 的 Playwright Electron 实验性支持；真实公开 Beta 制品只做对应平台 launch/smoke、fuse 与 manifest/hash 验证，不要求 Playwright attach。macOS job 在 arm64 runner 构建，先断言 `uname -m`，确保不产生 Intel 或 universal 制品。
**Rationale**

- Electron production fuse 会禁用 Node inspect arguments，Playwright 官方明确说明这会妨碍 Electron launcher 连接。将 integration 固定在开发态、将公开 Beta 制品放在独立 smoke 车道，既能测试行为，也不为测试放宽发行安全设置。
- native save dialog 不能由 renderer 自动化直接接管；主进程 integration 可在测试装配中观察或替代系统对话框，而公开 Beta 仍验证真实用户可见流程。
- 独立平台车道能阻止“网页测试全绿、目标平台包不可用”的结果进入 GitHub Pre-release。
  **Alternatives Rejected**

- 只做人工 Windows 测试：结果不可重复，且不能成为公开 Beta 候选的稳定门槛。
- 为让 Playwright 连接而在公开 Beta 包中重新开启 inspect fuse：扩大可攻击面，与桌面安全基线冲突。
- 在生产代码中增加 test-only IPC/开关拦截 native dialog：会污染远程零桥接边界；测试替身必须留在开发测试装配。
  **References**

- [Playwright Electron API](https://playwright.dev/docs/api/class-electron)
- [Electron Fuses](https://www.electronjs.org/docs/latest/tutorial/fuses)

## Decision 11: 服务端兼容契约先于公开 Beta 发布

**Decision**

兼容性 API 和 document security headers 必须先经既有 server deployment route 发布并在固定 production Origin 验证，随后才可创建公开 Beta Release。若 server 必须回退到缺少这些能力的版本，先暂停相应 GitHub Pre-release；客户端保持 fail closed 到 recovery。
**Rationale**

- 桌面宿主把 compatibility API 作为启动安全门。先发包、后上线 endpoint 会让首批用户得到无法判断的失败状态，排障成本高且易诱发临时 fallback。
- 把 server compatibility 与 headers 的线上验证作为 desktop release 的前置证据，能保证同一候选版本的客户端和服务端契约已经交汇。
- 回退时暂停 GitHub Pre-release 比让客户端接受旧协议、HTTP 或备用 Origin 更安全，也保持“固定唯一 Origin、严格 DTO”的长期边界。
  **Alternatives Rejected**

- 先创建公开 Beta、随后补 API/header：会制造不可重复的跨版本窗口。
- 允许客户端兼容旧 response 或备用服务地址：削弱 fail-closed 与固定 Origin 约束。
- 为 desktop 开辟旁路 server deploy：会绕开项目既有生产发布治理。
  **References**
...
1. 鐢熶骇椤甸潰鐨勫畬鏁?CSP resource inventory锛圢ext runtime銆丅lob 鍥惧儚銆佸瓧浣?鍥炬爣鍜屾棦鏈夊悎娉?connect source锛夈€?2. 鍚庣画姝ｅ紡鍏紑鐗堟湰鐨勪唬鐮佺鍚嶄緵搴斿晢銆佸彲淇＄鍚嶈处鎴枫€乼imestamp 鏈嶅姟涓?CI secret binding锛涜繖浜涗笉灞炰簬 v0.5.0 鍐呴儴棰勮鑼冨洿锛屼篃涓嶅啓鍏ヤ粨搴撱€?3. Windows x64 runner 涓婂唴閮ㄩ瑙堝埗鍝佺殑鏈鍚嶆爣璇嗐€乫use/hash 楠岃瘉璁板綍涓庨娆″畨瑁呫€佽鐩栧畨瑁?smoke 璇佹嵁銆?4. Windows x64 clean install 涓?Electron/Forge 鐨勬渶灏?pnpm `allowBuilds` 娓呭崟锛涘繀椤荤簿纭褰曪紝绂佹涓轰笅杞?binary 鎵撳紑瀹芥硾鑴氭湰鏉冮檺銆?
   ...
   当前本机结果（2026-08-03）：冻结安装先以 `electron: true` 的最小许可完成；安装日志明确识别 Squirrel.Windows 的直接构建依赖 `electron-winstaller`，因此只新增该精确许可。默认 Electron binary 下载源曾返回 `fetch failed`，但在一次性 `ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/` HTTPS mirror 环境中，`electron --version` 成功返回 `v43.2.0`；该变量没有写入 package、runtime config 或公开 Beta 制品。`electron-forge --version` 成功返回 `7.11.2`。将 `.artifacts/desktop/pnpm-builds-windows-x64.json` 附入本版本的 CI/acceptance evidence，并在记录中保留 `enabled` 与 `denied` 的完整包名列表、Electron/Forge 版本和执行时间；只有直接支持该安装的包可为 `enabled`，其余 package scripts 必须保持明确拒绝。
