...
v0.4.6 在 v0.4.5 `UserMemory Store` 基础上，为当前 browser session scope 内的长期 `UserMemory` 增加语义召回基线。本版确定只做 `PostgresStore` vector semantic search 作为正式召回路径，让“换一种说法”的用户请求也能召回相关长期记忆。rule-based / lexical candidate source 不属于本版正式召回路径、fallback 或验收路径。

技术路线保持在现有 `apps/webapp` runtime 的 `user-memory` 边界内：继续使用 LangGraph `BaseStore` / `PostgresStore`，对干净、校验后的 `UserMemory.text/tags` 建立 semantic vector index，`UserMemory.type` 只作为过滤、排序和展示元信息；latest user input 只作为临时 query；召回结果必须经过 browser-session namespace、active/confidence/suppression、topK 和 context budget 过滤后，才能作为 supplemental context 注入 ordinary text chat、tool-assisted ordinary chat，以及仍处于 ordinary chat boundary 内的 runtime-controlled capability-context final answer stage。
...
**Primary Dependencies**: `@langchain/langgraph` 1.3.6 (`BaseStore`), `@langchain/langgraph-checkpoint-postgres` 1.0.3 (`PostgresStore` semantic search types), `@langchain/core`, `@langchain/openai` existing dependency for 火山引擎 Ark OpenAI-compatible embedding integration style, `zod`, `@ai-mind/stream-core`, Vitest

**Storage**: 继续使用 v0.4.5 独立 `langgraph_user_memory` LangGraph Store schema。local development、integration、preview、staging、production 和真实语义验证统一使用 `PostgresStore` semantic vector search 配置。`PostgresStore` 的本地类型显示 semantic index 底层需要 vector search support；这属于 LangGraph Store provider 的存储能力要求，不把 `PGVectorStore`、独立 pgvector store 或外部向量库作为 AI Mind v0.4.6 产品路线。真实 semantic retrieval 只承诺在 `PostgresStore` 下工作；自动化测试如需稳定召回行为，只能在测试侧使用 fake / mocked store/search 或显式 test doubles，正式实现不得保留测试专用 runtime 分支。

**Testing**: 以 `apps/webapp/tests/lib/ai/runtime/user-memory-*` focused tests 为主，补充 semantic retrieval filtering/budget tests、semantic index content allowlist tests、embedding provider failure tests、no-query-rewrite tests、orchestrator eligibility tests、tool-assisted ordinary chat tests、Tasklist / Delivery non-regression tests，以及正式 runtime path 无 rule-based / lexical retrieval 的验证。最后执行 `pnpm --dir apps/webapp test`、`pnpm typecheck` 和 `pnpm lint:webapp`。
...
- semantic score 默认以 `>= 0.32` 作为最终注入阈值；该值基于 `doubao-embedding-vision` 在本版中文 UserMemory 场景下的真实分数分布校准；score 缺失或异常时保守降权或不注入。
...
- semantic retrieval 必须在执行 embedding query 或 Store semantic search 之前先判断 runtime path eligibility。Tasklist、Delivery、HITL checkpoint/resume、workflow progress、hydration、sidebar list 和 conversation switching 不得触发 semantic query。对 MCP / remote capability 场景，禁止的是 raw fetch/input path；仍处于 ordinary chat boundary 内的 runtime-controlled capability-context final answer stage 不视为这条排除项。
...
- `getUserMemoryRuntimeConfig()` 在 local development、integration、preview、staging 和 production 下必须默认返回 `PostgresStore` runtime path，不再提供 development 默认 `memory` mode。
...
- v0.4.6 的正式代码链路、正式验收测试依赖和正式 config path 中不得保留 rule-based / lexical runtime wiring。
...
- automated tests 使用测试侧 fake / mocked store/search 或其他显式 test doubles 产生稳定可预测结果，不依赖外部 API、网络、费用或非确定语义结果；该能力不作为 local development runtime。
- integration / acceptance 在具备 `DATABASE_URL`、Store setup、embedding provider key 和网络条件时验证真实 semantic recall；这也是 local development 的默认运行路径。
...
- automated tests 使用测试侧 fake / mocked store/search 或其他显式 test doubles，避免外部 API、网络、费用和语义漂移；这类测试替身只存在于测试代码，不作为应用运行时默认 Store。
...
- baseline retrieval 默认值固定为：semantic score 阈值 `0.32`、candidate topK `8`、query 长度上限 `800` 字符且使用前 `400` + 后 `400` 的确定性裁剪。
- semantic index metadata 第一版写入 UserMemory 内部 document metadata，不采用 runtime-only 临时状态。
- 正式实现不保留 `InMemoryStore`、deterministic embeddings、测试专用 provider kind 或 retrieval helper；development 默认 runtime path 也必须走 `PostgresStore`。
...
- `ChatOrchestrator` 必须在 UserMemory retrieval 前确认当前请求属于 ordinary text chat、ordinary chat boundary 内的 tool-assisted ordinary chat，或仍处于 ordinary chat boundary 内的 runtime-controlled capability-context final answer stage；被排除路径不得触发 semantic query。
- context builder 继续只接收最终 selected UserMemory，并保持 v0.4.5 的 bounded supplemental system context。
- provider / retrieval 的测试替身能力必须停留在测试侧，正式 runtime 默认分支不得继续感知测试专用 provider/store/search path。
...
- 正式实现不再保留 `InMemoryStore` local runtime、deterministic helper、测试专用 `storeMode` 或 provider/retrieval 分支；测试若需稳定语义行为，只能在测试代码侧显式注入 fake / mocked store/search。
...
- 第一版默认接受阈值为 `semantic score >= 0.32`，初始 candidate topK 为 `8`。
...
- 当前 `chat-orchestrator.ts` 已经有 `isUserMemoryContextEligibleRequest()`，但 v0.4.6 必须确保该判断足够区分 ordinary chat、tool-assisted ordinary chat、`reader-skill` capability-context final answer stage 与 Tasklist / Delivery，且发生在任何 semantic provider 调用之前。
