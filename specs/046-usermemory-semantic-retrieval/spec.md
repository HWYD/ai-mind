...
- v0.4.6 只使用 `PostgresStore` vector semantic search 作为正式召回路径，不使用 rule-based / lexical / metadata candidate source，也不使用 `PostgresStore` hybrid/text search。
- semantic retrieval 默认启用，不依赖 feature flag。
- ordinary text chat 和 ordinary chat boundary 内的最终回答路径都属于正式范围；其中包含 tool-assisted ordinary chat，也包含 `reader-skill` 的 runtime-controlled capability-context final answer stage。Tasklist / Delivery 等路径继续排除在外。
- semantic index 只允许使用 `UserMemory.text` 和 `UserMemory.tags`；`UserMemory.type` 只作为过滤、排序和展示元信息。
- 真实 embedding provider 使用独立的 UserMemory runtime config，固定走火山引擎 Ark OpenAI-compatible 路线，model id 固定为 `doubao-embedding-vision`。
- semantic retrieval 默认超时为 `1500ms`，初始 `topK = 8`，最终注入阈值为 `semantic score >= 0.32`。
- retrieval query 直接使用 latest user input，只允许做确定性的 trim、空白折叠和长度裁剪；最多 800 字符，超出时保留前 400 字符和后 400 字符；不做 LLM query rewrite。
- 当路径边界无法安全判断、semantic score 缺失/异常，或 semantic retrieval 失败时，统一按 0 条 UserMemory 注入处理。
- 自动化测试可以在测试侧使用 fake / mocked store/search 或显式 test doubles，但正式实现不保留测试专用 runtime 分支。
...
对于仍属于 ordinary chat 边界的 tool-assisted 普通问答，系统 MUST 使用同一套 UserMemory semantic retrieval 规则。UserMemory 只能影响普通回答上下文，不能改变 Tool、MCP raw fetch/input、Tasklist 或 Delivery 的权限边界。

**Why this priority**: tool-assisted ordinary chat 是普通问答正式范围的一部分；如果这一类请求不进入正式验收，用户的长期偏好体验会在常见工具场景里出现断层。

**Independent Test**: 在 tool-assisted ordinary chat 或仍处于 ordinary chat boundary 内的 `reader-skill` capability-context 最终回答阶段提出与长期偏好相关的问题。验证 UserMemory 可以作为最终回答上下文使用，但不改变工具调用权限、raw tool input、MCP raw fetch/input 或 Tasklist / Delivery 边界。
...
2. **Given** UserMemory 被选中，**When** 工具调用或 runtime-controlled capability context 执行，**Then** UserMemory 不得改变 tool authority、raw tool input 或 MCP raw fetch/input 的安全边界。
...
- **FR-046-009A**: 当 `reader-skill` 的 runtime-controlled capability-context final answer stage 仍处于 ordinary chat context boundary 内时，System MAY 复用同一套 UserMemory semantic retrieval 上下文装配规则；但 semantic retrieval MUST NOT 流入该 capability 的 raw resource / raw prompt fetch/input path。
- **FR-046-010**: System MUST NOT 默认在 Tasklist Agent、Delivery Chain、Delivery Manager、HITL checkpoint / resume path、workflow progress path、frontend hydration、sidebar conversation list 或 conversation switching 中启用 UserMemory semantic retrieval。
...
- **FR-046-014B**: v0.4.6 的最终 injected candidate MUST 满足 `semantic score >= 0.32`；该默认值基于 `doubao-embedding-vision` 在本版 UserMemory 文本场景下的本地校准，后续模型或检索策略变化时 MUST 重新校准。当 score 缺失、异常或不稳定时，System MUST 保守降权或不注入。
...
- **FR-046-017D**: v0.4.6 的正式代码链路和正式验收路径 MUST NOT 使用 rule-based / lexical retrieval 作为 candidate source、fallback 或 hybrid 预实现。
...
- **FR-046-020C**: 真实 semantic retrieval MUST 只承诺 `PostgresStore`。自动化测试 MAY 在测试侧使用 fake / mocked store/search 或显式 test doubles 验证召回策略，但这些 MUST NOT 被表达为真实线上 semantic search 能力。
- **FR-046-020E**: v0.4.6 的 local development、integration、preview、staging 和 production runtime MUST 默认使用 `PostgresStore`。正式实现 MUST NOT 为测试保留专门的 `storeMode`、`providerKind`、deterministic semantic helper 或 retrieval 分支。
...
- **FR-046-068**: System MUST 将 tool-assisted ordinary chat 纳入 v0.4.6 的正式验收范围；当 `reader-skill` capability-context final answer stage 仍处于 ordinary chat context boundary 内时，它 MAY 复用同一套 UserMemory context assembly 规则，但其接入不得改变 Tool、MCP raw fetch/input、Tasklist 或 Delivery 的权限边界。
...
- **SC-046-015**: v0.4.6 的正式代码链路和正式验收路径中，rule-based / lexical retrieval 被 runtime 触发的次数为 0。
...
- automated tests 需要稳定语义行为，但测试侧 fake / mocked store/search 或其他显式 test doubles 不代表线上产品能力。
- 真实 semantic retrieval 只承诺 `PostgresStore`；正式实现不保留仅为测试服务的 runtime store/provider/retrieval 分支。
...
- v0.4.6 的默认 semantic score 阈值为 `0.32`，初始 topK 为 `8`。
