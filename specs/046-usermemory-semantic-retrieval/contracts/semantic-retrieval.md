...
- MUST NOT 在 Tasklist、Delivery、HITL checkpoint/resume、workflow progress、MCP raw resource path、hydration、sidebar list 或 conversation switching 中触发 semantic retrieval。
...
- `InMemoryStore` MAY be used for deterministic tests, but MUST NOT be treated as real semantic retrieval.
- MUST 以 `stableKey` 去重。
...
- `semanticScore < 0.70` MUST NOT enter the final selected list.
...
- legacy rule-based / lexical runtime wiring MUST be removed from formal runtime path and formal acceptance dependency by v0.4.6 release closing.
