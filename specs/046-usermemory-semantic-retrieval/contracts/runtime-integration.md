...
- runtime-controlled capability-context final answer stage that still stays inside ordinary chat context boundary, including `reader-skill`
...
- UserMemory MUST NOT be exposed to MCP raw resource / raw prompt fetch/input path.
- UserMemory MUST NOT alter Tasklist / Delivery routing.

When the request enters a runtime-controlled capability-context final answer stage:

- If it still stays inside ordinary chat context boundary, UserMemory MAY reuse the same final answer context assembly.
- UserMemory MUST NOT alter the capability's raw fetch/input path.
