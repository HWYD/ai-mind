...
- 自动化测试 MAY 在测试侧使用 fake / mocked store/search 或显式 test doubles，但正式实现 MUST NOT 为此增加专门的 runtime store/provider/retrieval 分支。
...
- `semanticScore < 0.32` MUST 丢弃。
- score 缺失或不稳定时 SHOULD 保守降权或丢弃。

## Retrieval Source Boundary

v0.4.5 的 rule-based retrieval 不属于 v0.4.6 data model 的正式 candidate entity。
...
- v0.4.6 的正式 runtime path、正式 acceptance path 和正式 config path MUST NOT 使用 rule-based / lexical retrieval。
...
- automated tests MUST 使用测试侧 fake / mocked store/search 或其他显式 test doubles，正式实现 MUST NOT 为测试保留专门的 provider/store/search 分支。
