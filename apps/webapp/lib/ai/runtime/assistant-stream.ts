...
function stripReasoningMetadata(additionalKwargs?: Record<string, unknown>) {
    if (!additionalKwargs || !('reasoning_content' in additionalKwargs)) return additionalKwargs
...
export async function streamAssistantParts(
    ...
    const ensureTextPartStarted = () => {
        ...
        textStarted = true
        writeChunk({ type: 'text-start', partId: textPartId })
    }
    const ensureReasoningPartStarted = () => {
        if (reasoningStarted) return
        reasoningStarted = true
        writeChunk({ type: 'reasoning-start', partId: reasoningPartId })
    }
    for await (const chunk of modelStream) {
        if (context.signal?.aborted || isClosed()) {
            if (context.signal?.aborted) logChatCancellation('request aborted by client')
            return
        }
        ...
        if (emitReasoning && reasoning) {
            ...
            writeChunk({ type: 'reasoning-delta', partId: reasoningPartId, delta: reasoning })
        }
        if (text) {
            completedText += text
            ensureTextPartStarted()
            writeChunk({ type: 'text-delta', partId: textPartId, delta: text })
        }
    }
    if (emitReasoning && reasoningStarted) writeChunk({ type: 'reasoning-end', partId: reasoningPartId })
    if (textStarted) writeChunk({ type: 'text-end', partId: textPartId })
    return completedText
}

function toAIMessage(chunk: AIMessageChunk | null, includeReasoningMetadata = true): AIMessage {
    if (!chunk) return new AIMessage({ content: '', tool_calls: [], invalid_tool_calls: [] })
...
export async function streamPlanningResponse(
    ...
    for await (const chunk of modelStream) {
        ...
        if (chunk.tool_call_chunks?.length || chunk.tool_calls?.length) encounteredToolCall = true
        const text = getChunkText(chunk)
