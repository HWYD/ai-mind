const TOOL_DESCRIPTIONS: Record<string, string> = {
    ...
    datetime: [
        ...
        '动作选择：查询当前时间或当前星期时优先用 action=now；查询某个明确日期是星期几时优先用 action=weekday；查询相对日期、日期加减、几天后、几小时后、下周几这类偏移计算时优先用 action=add。',
        '必须调用：只要用户在问今天、现在、明天、后天、下周几、几天后、几小时后、某个日期是星期几，并且当前可用工具里包含 datetime，就必须优先调用 datetime。',
        '不要误用：不要把抽象的时间讨论误判成工具问题，也不要自己心算相对日期或星期。',
    ].join(' '),
    'local-text-read': [
        'local-text-read：只负责读取项目根目录的直接文本文件内容。',
        '适用：读取 README.md、查看 package.json、总结 notes.txt。',
        '必须调用：当用户明确给出根目录文件名，并要求读取、查看、总结或提取该文件内容时。',
        '不要误用：不要生成子目录路径、绝对路径或 ../，也不要猜测不存在的文件。',
    ].join(' '),
    'unit-convert': [
...
export function getToolUseSystemPrompt(activeToolNames: string[]) {
    ...
    return [
        ...
        '如果用户要的是确定性结果、实时信息、文件内容、结构化转换结果，优先考虑工具；如果用户要的是观点、解释或开放式表达，则可以直接回答。',
        '',
        ...
        '- 如果用户明确给出根目录文件名，并要求读取、查看、总结或提取该文件内容，并且当前可用工具里包含 local-text-read，必须优先调用 local-text-read。',
        '- local-text-read 的参数应尽量只传文件名，不要生成子目录路径、绝对路径或 ../。',
...
export function getToolRetrySystemPrompt(activeToolNames: string[]) {
    ...
    return [
        ...
        '5. 如果用户已经明确要求实时天气、读取根目录文件、格式转换、精确计算、日期推断或单位换算，不要再回避工具。',
        '',
        toAvailableToolsSection(activeToolNames),
        '',
        '补充规则：',
        '- 天气请求如果当前可用工具里包含 city-weather，应优先调用它，而不是自己猜测天气结果。',
        '- 文件读取请求如果当前可用工具里包含 local-text-read，应优先调用它，而不是假装看过本地文件。',
        '- local-text-read 的参数应尽量只传文件名，不要生成路径。',
...
export function getToolResultSystemPrompt(activeToolNames: string[]) {
    ...
    return [
        ...
        '- 对 city-weather 和 local-text-read 的结果，优先保留工具返回的事实信息，不要编造额外上下文。',
