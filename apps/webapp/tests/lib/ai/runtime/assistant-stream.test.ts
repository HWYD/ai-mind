...
describe('assistant-stream', () => {
    it('关闭深度思考时不写出 reasoning chunk', async () => {
        ...
        await streamAssistantParts(
            toAsyncChunks([new AIMessageChunk({ content: '最终答案', additional_kwargs: { reasoning_content: '这里是思考过程' } })]),
            {},
    ...
    it('planning 阶段关闭深度思考时会清理 reasoning metadata', async () => {
        ...
        const response = await streamPlanningResponse(
            toAsyncChunks([
                new AIMessageChunk({
                    content: '先调用工具',
                    additional_kwargs: { reasoning_content: '这里是规划思考', trace_id: 'trace-1' },
                }),
            ]),
            {},
            chunk => writtenChunks.push(chunk),
    ...
    it('只累计可见文本，忽略混入 content 数组中的非文本中间 parts', async () => {
        ...
        expect(writtenChunks).toContainEqual(expect.objectContaining({ type: 'text-delta', delta: '最终回答第一段。最终回答第二段。' }))
    })
})
