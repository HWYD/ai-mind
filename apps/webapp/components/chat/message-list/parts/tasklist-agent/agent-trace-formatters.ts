import type { AgentGraphPart, PromptPart, ResourcePart, ToolPart } from '@/lib/ai/types/message'

export type DetailRuntimeStatus = AgentGraphPart['status'] | PromptPart['status'] | ResourcePart['status'] | ToolPart['status']
...
export function getAgentStatusLabel(status: AgentGraphPart['status']) {
    switch (status) {
        case 'completed':
            return '已完成'
        case 'failed':
