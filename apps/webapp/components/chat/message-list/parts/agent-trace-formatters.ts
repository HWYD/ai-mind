    ...
    switch (key) {
        case 'action': {
            const actionLabels: Record<string, string> = {
                ask_clarification: '决策：需要澄清',
                proceed_to_tasklist_strategy: '决策：继续生成',
                proceed_with_manual_review_items: '决策：带复核继续',
                read_optional_context: '决策：读取补充上下文',
                stop_with_boundary_message: '决策：边界停止',
            }

            return actionLabels[value] ?? `决策：${value}`
        }
        case 'decision': {
            const decisionLabels: Record<string, string> = {
                blocked: '最终决策：阻塞',
                final: '最终决策：可采用',
                final_with_manual_review_items: '最终决策：需复核',
            }

            return decisionLabels[value] ?? `最终决策：${value}`
        }
        case 'fixnow':
            return `自动修正：${value}`
        case 'granularity':
