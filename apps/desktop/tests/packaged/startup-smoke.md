...
**Scope**: T043 Safe MVP manual acceptance. This record is required evidence for the unsigned `public-beta` GitHub Pre-release after the applicable server-first gate has passed.
...
1. On Windows x64, download the unsigned `public-beta` installer from the same-commit GitHub Pre-release and install it into a clean Windows user profile. Do not redistribute it through another channel.
...
6. Open `AI Mind Desktop` > `关于 AI Mind Desktop`. Confirm the Chinese About dialog shows the desktop version, `public-beta`, `unsigned`, and the fixed Trusted Origin. It must not load remote content or offer an upgrade URL.
...
2. Install a newer unsigned `public-beta` candidate with the same product identity over the existing installation for the same Windows user. Confirm the restarted application retains the existing desktop profile and only exposes data authorized by the server session.
...
1. On an Apple Silicon Mac, download and mount the same-commit unsigned `public-beta` DMG from the GitHub Pre-release only after the server-first gate has passed. Confirm the manifest platform is `darwin-arm64` and its SHA-256 matches the DMG.
